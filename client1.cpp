...
int main () {
	...
	if (status != 0) {
		perror("Failed to retrieve client1:getaddrinfo");
		return 1;
	}
	...
	if (sockD == -1) {
		perror("Failed to create client1:socket");
		return 1;
	}
	...
	int bindStat = bind(sockD, (struct sockaddr*)&c, addr_len);
	  if (bindStat == -1) {
	  	perror("Failed to bind client1:bind");
	  	return 1;
	  }
	...
	if (getsockstat == -1) {
		perror("Failed to retrieve port number client1:getsockname");
		return 1;
	}
	...
	if (sentBytes == -1) {
		perror("Failed to send client1:send");
		return 1;
	}
	else if (sentBytes != len) {
		perror("Not all bytes sent client1:send");
		return 1;
	}
